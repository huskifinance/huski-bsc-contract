...
contract PancakeswapV2WithdrawMinimizeTrading is OwnableUpgradeSafe, ReentrancyGuardUpgradeSafe, IStrategy {
  ...
  modifier onlyWhitelistedWorkers() {
    require(okWorkers[msg.sender], "PancakeswapV2WithdrawMinimizeTrading::onlyWhitelistedWorkers: bad worker");
    _;
  } 
  ...
  function execute(address user, uint256 debt, bytes calldata data) external override onlyWhitelistedWorkers nonReentrant {
    ...
    require(lpToken.approve(address(router), uint256(-1)), "PancakeswapV2WithdrawMinimizeTrading::execute: failed to approve LP token");
    ...
    require(remainingFarmingToken >= minFarmingToken, "PancakeswapV2WithdrawMinimizeTrading::execute: insufficient farming tokens received");
    ...
    require(lpToken.approve(address(router), 0), "PancakeswapV2WithdrawMinimizeTrading::execute: unable to reset lp token approval");
