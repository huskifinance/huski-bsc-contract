...
/// @notice The (older) FairLaunch contract lock and vest mechanic embeded in HuskyToken
/// Hence to make the protocol to be able to adjust those lock and vest mechanic we need FairLaunchV2
/// However, FairLaunch is the only owner with the right to mint HuskyToken,
/// therefore newly minted Husky will go out from FairLaunch but then FairLaunchV2 will be the only one
...
contract FairLaunchV2 is Ownable, ReentrancyGuard {
  ...
  /// @notice The HUSKY ERC-20 contract.
  IERC20 public immutable HUSKY;
  ...
  uint256 private constant ACC_HUSKY_PRECISION = 1e12;
  ...
  event LogUpdatePool(uint256 indexed pid, uint256 lastRewardBlock, uint256 lpSupply, uint256 accHuskyPerShare);
  event LogInit();

  /// @notice Harvests HUSKY from `FAIR_LAUNCH` and pool `MASTER_PID` to this contract.
  ...
  /// @param _husky the HUSKY Token
  /// @param _MASTER_PID the pool ID of the dummy token on the base contract
  constructor(IFairLaunchV1 _FAIR_LAUNCH_V1, IERC20 _husky, uint256 _MASTER_PID) public {
  ...
  /// This is required because `FAIR_LAUNCH` holds the minting rights for HUSKY.
  ...
  /// @notice Update the given pool's HUSKY allocation point and `ILocker` contract. Can only be called by the owner.
  ...
  /// @notice View function to see pending HUSKYs on frontend.
  /// @param _pid The index of the pool. See `poolInfo`.
  /// @param _user address of user
  function pendingHusky(uint256 _pid, address _user) external view returns (uint256) {
    ...
    if (block.number > pool.lastRewardBlock && stakeTokenSupply != 0) {
      ...
      uint256 huskyReward = blocks.mul(huskyPerBlock()).mul(pool.allocPoint).mul(ACC_HUSKY_PRECISION).div(totalAllocPoint);
      accHuskyPerShare = accHuskyPerShare.add(huskyReward.div(stakeTokenSupply));
    }
    uint256 _pendingHusky = (user.amount.mul(accHuskyPerShare) / ACC_HUSKY_PRECISION).sub(user.rewardDebt);
    return _pendingHusky;
  }
  ...
  /// @notice calculates the `amount` of HUSKY per block
  function huskyPerBlock() public view returns (uint256 amount) {
    amount = uint256(FAIR_LAUNCH_V1.huskyPerBlock())
  ...
  function updatePool(uint256 pid) public harvestFromFairLaunchV1 returns (PoolInfo memory pool) {
    ...
    if (block.number > pool.lastRewardBlock) {
      ...
      if (stakeTokenSupply > 0 && totalAllocPoint > 0) {
        uint256 blocks = block.number.sub(pool.lastRewardBlock);
        uint256 huskyReward = blocks.mul(huskyPerBlock()).mul(pool.allocPoint).mul(ACC_HUSKY_PRECISION).div(totalAllocPoint);
        pool.accHuskyPerShare = pool.accHuskyPerShare.add((huskyReward.div(stakeTokenSupply)));
      }
      pool.lastRewardBlock = block.number;
      poolInfo[pid] = pool;
      emit LogUpdatePool(pid, pool.lastRewardBlock, stakeTokenSupply, pool.accHuskyPerShare);
    }
  }

  /// @notice Deposit LP tokens to MasterChef for HUSKY allocation.
  ...
  function deposit(address _for, uint256 pid, uint256 amount) external harvestFromFairLaunchV1 nonReentrant {
    ...
    user.rewardDebt = user.rewardDebt.add(amount.mul(pool.accHuskyPerShare) / ACC_HUSKY_PRECISION);
  ...
  function withdraw(address _for, uint256 pid, uint256 amount) external nonReentrant {
    ...
    user.rewardDebt = user.rewardDebt.sub(amount.mul(pool.accHuskyPerShare) / ACC_HUSKY_PRECISION);
  ...
  /// @notice Harvest HUSKYs earn from the pool.
  ...
  /// @param pid The index of the pool. See `poolInfo`.
  /// @param to Receiver of HUSKY rewards.
  function _harvest(address to, uint256 pid) internal harvestFromFairLaunchV1 {
    PoolInfo memory pool = poolInfo[pid];
    UserInfo storage user = userInfo[pid][to];
    uint256 accumulatedHusky = user.amount.mul(pool.accHuskyPerShare).div(ACC_HUSKY_PRECISION);
    uint256 _pendingHusky = accumulatedHusky.sub(user.rewardDebt);
    if (_pendingHusky == 0) { return; }

    require(_pendingHusky <= HUSKY.balanceOf(address(this)), "FairLaunchV2::_harvest: wtf not enough husky");

    // Effects
    user.rewardDebt = accumulatedHusky;

    // Interactions
    ILocker _locker = lockers[pid];
    if (address(_locker) != address(0)) {
      uint256 lockAmount = _locker.calLockAmount(_pendingHusky);
      HUSKY.safeApprove(address(_locker), lockAmount);
      _locker.lock(to, lockAmount);
      _pendingHusky = _pendingHusky.sub(lockAmount);
      HUSKY.safeApprove(address(_locker), 0);
    }

    HUSKY.safeTransfer(to, _pendingHusky);

    emit Harvest(msg.sender, pid, _pendingHusky);
  }
