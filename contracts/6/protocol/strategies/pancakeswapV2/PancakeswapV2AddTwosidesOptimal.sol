...
contract PancakeswapV2AddTwoSidesOptimal is OwnableUpgradeSafe, ReentrancyGuardUpgradeSafe, IStrategy {
  ...
  modifier onlyWhitelistedWorkers() {
    require(okWorkers[msg.sender], "PancakeswapV2AddTwoSidesOptimal::onlyWhitelistedWorkers: bad worker");
    _;
  }
  ...
  function _optimalDepositA(
    ...
    uint256 e = HuskyMath.sqrt(b.mul(b).add(d));
  ...
  function execute(
    ...
    (uint256 farmingTokenAmount, uint256 minLPAmount, uint256 serialCode) = abi.decode(data, (uint256, uint256, uint256));
    ...
    // 3. Compute the optimal amount of BaseToken and FarmingToken to be converted. And inject pos serial code to vault position record
    vault.requestFunds(farmingToken, farmingTokenAmount);
    vault.injectSerialCode(serialCode);
    ...
    require(moreLPAmount >= minLPAmount, "PancakeswapV2AddTwoSidesOptimal::execute: insufficient LP tokens received");
    require(
      lpToken.transfer(msg.sender, lpToken.balanceOf(address(this))),
      "PancakeswapV2AddTwoSidesOptimal::execute: failed to transfer LP token to msg.sender"
    );
