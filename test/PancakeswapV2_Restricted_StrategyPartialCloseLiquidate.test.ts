...
describe('PancakeswapV2PartialCloseLiquidate', () => {
  ...
  /// Strategy instance(s)
  let strat: PancakeswapV2PartialCloseLiquidate;
  ...
  let stratAsAlice: PancakeswapV2PartialCloseLiquidate;
  let stratAsBob: PancakeswapV2PartialCloseLiquidate;
  ...
  beforeEach(async () => {
    ...
    strat = await upgrades.deployProxy(PancakeswapV2PartialCloseLiquidate, [routerV2.address], { unsafeAllow: ['delegatecall'] }) as PancakeswapV2PartialCloseLiquidate;
    ...
    stratAsAlice = PancakeswapV2PartialCloseLiquidate__factory.connect(strat.address, alice);
    stratAsBob = PancakeswapV2PartialCloseLiquidate__factory.connect(strat.address, bob);
  ...
  context("When caller worker hasn't been whitelisted", async () => {
    it('should revert as bad worker', async () => {
      ...
      )).to.be.revertedWith('PancakeswapV2PartialCloseLiquidate::onlyWhitelistedWorkers: bad worker');
    });
  })
  ...
  it('should revert when the given LPs > the actual LPs sent to strategy', async () => {
    ...
    )).revertedWith('PancakeswapV2PartialCloseLiquidate::execute: insufficient LP amount recevied from worker')
  });
