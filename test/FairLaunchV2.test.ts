...
describe("FairLaunchV2", () => {
  ...
  beforeEach(async() => {
    ...
    huskyToken = await HuskyToken.deploy(0, 1);
    ...
    for(let i = 0; i < 4; i++) {
      const mockERC20 = await upgrades.deployProxy(MockERC20, 
        [`STOKEN${i}`, `STOKEN${i}`], 
        { unsafeAllow: ['delegatecall'] }) as MockERC20;
      await mockERC20.deployed();
      stakingTokens.push(mockERC20);
    }

    // Create fairLaunchLink token
    fairLaunchLink = await upgrades.deployProxy(MockERC20, 
      ['fairLaunchLink', 'fairLaunchLink'], { unsafeAllow: ['delegatecall'] }) as MockERC20
    ...
    fairLaunchV2 = await FairLaunchV2.deploy(
      fairLaunch.address, huskyToken.address, 1
    )
    await fairLaunch.deployed();
  ...
  context('when use pool', async() => {
    ...
    it('should revert when withdrawer is not a funder', async () => {
      ...
      await fairLaunch.addPool(0, stakingTokens[0].address, false);
      await fairLaunch.addPool(1, fairLaunchLink.address, false);
      await complexPriceOracle.setPrices([stakingTokens[0].address,fairLaunchLink.address],[1,1])
    ...
    it('should allow deposit when funder withdrew funds and owner want to stake his own token', async () => {
      ...
      let bobHuskyBalanceBefore = await huskyToken.balanceOf(await bob.getAddress());
      await fairLaunchV2AsBob.harvest(0);
      expect(await huskyToken.balanceOf(await bob.getAddress())).to.be.gt(bobHuskyBalanceBefore);
    ...
    it('should harvest yield from the position opened by funder', async () => {
      ...
      expect(await fairLaunchV2AsBob.pendingHusky(0, (await bob.getAddress()))).to.be.bignumber.eq(ethers.utils.parseEther('5000'));
    ...
    it('should distribute rewards according to the alloc point', async() => {
      ...
      expect(await fairLaunchV2.pendingHusky(0, (await alice.getAddress()))).to.be.bignumber.eq(ethers.utils.parseEther('7500'));
      expect(await fairLaunchV2.pendingHusky(1, (await alice.getAddress()))).to.be.bignumber.eq(ethers.utils.parseEther('2500'));

      // 6. Harvest all yield of pId 0
      // should get 7,500 + 2,500 = 10,000 HUSKIs from pId 0
      await fairLaunchV2AsAlice.harvest(0);
      expect(await huskyToken.balanceOf((await alice.getAddress()))).to.be.bignumber.eq(ethers.utils.parseEther('10000'));

      // 7. Harvest all yield of pId 1
      // should get 2,500 + 2,500 + 2,500 = 7,500 HUSKIs from pId 1
    ...
    it('should work', async() => {
      ...
      expect(await fairLaunchV2.pendingHusky(0, (await alice.getAddress()))).to.be.bignumber.eq(ethers.utils.parseEther('5000'));
      ...
      expect(await fairLaunchV2.pendingHusky(0, (await alice.getAddress()))).to.be.bignumber.eq(ethers.utils.parseEther('10000'));
      ...
      expect(await fairLaunchV2.pendingHusky(0, (await alice.getAddress()))).to.be.bignumber.eq(ethers.utils.parseEther('12500'));
      expect(await fairLaunchV2.pendingHusky(0, (await bob.getAddress()))).to.be.bignumber.eq(ethers.utils.parseEther('2500'));
      ...
      // Dev get 10% tax per block (5,000*0.1 = 500/block)
      expect(await fairLaunchV2.pendingHusky(0, (await alice.getAddress()))).to.be.bignumber.eq(ethers.utils.parseEther('15000'));
      expect(await fairLaunchV2.pendingHusky(0, (await bob.getAddress()))).to.be.bignumber.eq(ethers.utils.parseEther('5000'));
      expect(await huskyToken.balanceOf((await dev.getAddress()))).to.be.bignumber.eq(ethers.utils.parseEther('6000'));
      ...
      expect(await fairLaunchV2.pendingHusky(0, (await alice.getAddress()))).to.be.bignumber.eq(ethers.utils.parseEther('17500'));
      expect(await fairLaunchV2.pendingHusky(0, (await bob.getAddress()))).to.be.bignumber.eq(ethers.utils.parseEther('0'));
      expect(await huskyToken.balanceOf((await bob.getAddress()))).to.be.bignumber.eq(ethers.utils.parseEther('7500'));
      expect(await huskyToken.balanceOf((await dev.getAddress()))).to.be.bignumber.eq(ethers.utils.parseEther('6500'));
      ...
      // Alice should get 22,500 HUSKIs (17,500 + 2,500 [B1] + 2,500 [B2]) in pending Husky
      // Bob should has (2,500 [B1] + 2,500 [B2]) = 5,000 HUSKIs in pending
      expect(await fairLaunchV2.pendingHusky(0, (await alice.getAddress()))).to.be.bignumber.eq(ethers.utils.parseEther('22500'));
      expect(await fairLaunchV2.pendingHusky(0, (await bob.getAddress()))).to.be.bignumber.eq(ethers.utils.parseEther('5000'));
      expect(await huskyToken.balanceOf((await alice.getAddress()))).to.be.bignumber.eq(ethers.utils.parseEther('15000'));
      expect(await huskyToken.balanceOf((await bob.getAddress()))).to.be.bignumber.eq(ethers.utils.parseEther('7500'));
      expect(await huskyToken.balanceOf((await dev.getAddress()))).to.be.bignumber.eq(ethers.utils.parseEther('7500'));
      ...
      // Hence, Alice should get 22,500 + 4,000 = 26,5000 HUSKIs in pending
      // Bob should get 5,000 + 1,000 = 6,000 HUSKIs in pending
      expect(await fairLaunchV2.pendingHusky(0, (await alice.getAddress()))).to.be.bignumber.eq(ethers.utils.parseEther('26500'));
      expect(await fairLaunchV2.pendingHusky(0, (await bob.getAddress()))).to.be.bignumber.eq(ethers.utils.parseEther('6000'));
      expect(await huskyToken.balanceOf((await alice.getAddress()))).to.be.bignumber.eq(ethers.utils.parseEther('15000'));
      expect(await huskyToken.balanceOf((await bob.getAddress()))).to.be.bignumber.eq(ethers.utils.parseEther('7500'));
      expect(await huskyToken.balanceOf((await dev.getAddress()))).to.be.bignumber.eq(ethers.utils.parseEther('8000'));
    });
    ...
    it('should work when there is a locker', async() => {
      ...
      const linearRelease = await LinearRelease.deploy(
        huskyToken.address, '7000', fairLaunchV2.address, startReleaseBlock, endReleaseBlock) as LinearRelease
      ...
      // 6. Check pendingHusky for Alice
      expect(await fairLaunchV2.pendingHusky(0, (await alice.getAddress()))).to.be.bignumber.eq(ethers.utils.parseEther('5000'));

      // 7. Trigger random update pool to make 1 more block mine
      await fairLaunchV2AsAlice.massUpdatePools([0]);

      // 8. Check pendingHusky for Alice
      expect(await fairLaunchV2.pendingHusky(0, (await alice.getAddress()))).to.be.bignumber.eq(ethers.utils.parseEther('10000'));

      // 9. Alice should get 15,000 * (1-0.7) = 4,500 HUSKIs when she harvest
      // also check that dev got his tax
      // PS. Dev get 4,500 as MASTER_POOL was added 9 blocks earlier
      await fairLaunchV2AsAlice.harvest(0);
      expect(await huskyToken.balanceOf((await alice.getAddress()))).to.be.bignumber.eq(ethers.utils.parseEther('4500'));
      expect(await huskyToken.balanceOf((await dev.getAddress()))).to.be.bignumber.eq(ethers.utils.parseEther('4500'));
      ...
      // Reward per Block is till (50-50) as Bob is not leaving the pool yet
      // Alice should has 17,500 HUSKIs (15,000 + 2,500) in pending
      // Bob should has 7,500 * 0.3 = 2,250 HUSKIs (5,000 + 2,500) in his account as he harvest it
      // Dev get 10% tax per block (5,000*0.1 = 500/block)
      await fairLaunchV2AsBob.harvest(0);

      expect(await fairLaunchV2.pendingHusky(0, (await alice.getAddress()))).to.be.bignumber.eq(ethers.utils.parseEther('17500'));
      expect(await fairLaunchV2.pendingHusky(0, (await bob.getAddress()))).to.be.bignumber.eq(ethers.utils.parseEther('0'));
      expect(await huskyToken.balanceOf((await bob.getAddress()))).to.be.bignumber.eq(ethers.utils.parseEther('2250'));
      expect(await huskyToken.balanceOf((await dev.getAddress()))).to.be.bignumber.eq(ethers.utils.parseEther('7000'));
      expect(await linearRelease.lockOf(await alice.getAddress())).to.be.bignumber.eq(ethers.utils.parseEther('10500'));
      ...
      expect(await huskyToken.balanceOf((await alice.getAddress()))).to.be.bignumber.eq(ethers.utils.parseEther('8700'));
      expect(await huskyToken.balanceOf((await bob.getAddress()))).to.be.bignumber.eq(ethers.utils.parseEther('2250'));
      ...
      expect(await huskyToken.balanceOf((await alice.getAddress()))).to.be.bignumber.eq(ethers.utils.parseEther('8700'));
      expect(await huskyToken.balanceOf((await bob.getAddress()))).to.be.bignumber.eq(ethers.utils.parseEther('5400'));
  ...
  context('when migrate from FLV1 to FLV2', async() => {
    it('should migrate successfully', async() => {
      ...
      // 9. Bob come in and join the party
      // 2 blocks are mined here, hence Alice should get 10,000 HUSKIs more (2x5,000)
      ...
      expect(await huskyToken.balanceOf(await alice.getAddress())).to.be.bignumber.eq(ethers.utils.parseEther('30000'));
      expect(await huskyToken.balanceOf(await bob.getAddress())).to.be.bignumber.eq(ethers.utils.parseEther('5000'));
      ...
      // 17. Check pendingHusky for Alice & Bob. Expected to be 0 as pool has no allocPoint yet
      // expect dev token to be the same amount as no husky has been mint
      expect(await fairLaunchV2.pendingHusky(0, await alice.getAddress())).to.be.bignumber.eq(ethers.utils.parseEther('0'));
      expect(await fairLaunchV2.pendingHusky(0, await alice.getAddress())).to.be.bignumber.eq(ethers.utils.parseEther('0'));
      expect(await huskyToken.balanceOf(await dev.getAddress())).to.be.bignumber.eq(ethers.utils.parseEther('3500'));

      await fairLaunchV2.init(fairLaunchLink.address);

      // 18. Set allocPoint to 1 to continue mintting HUSKIs
      ...
      // 20. Now Alice and Bob should get some HUSKIs
      // Check pendingHusky for Alice & Bob
      expect(await fairLaunchV2.pendingHusky(0, (await alice.getAddress()))).to.be.bignumber.eq(ethers.utils.parseEther('7500'));
      expect(await fairLaunchV2.pendingHusky(0, (await alice.getAddress()))).to.be.bignumber.eq(ethers.utils.parseEther('7500'));
      expect(await huskyToken.balanceOf((await dev.getAddress()))).to.be.bignumber.eq(ethers.utils.parseEther('4500'));

      // 21. Alice should get 10,000 HUSKIs when she harvest
      // also check that dev got his tax
      // PS. Dev get 5,000 as mining continue 2 blocks earlier
      await fairLaunchV2AsAlice.harvest(0);
      expect(await huskyToken.balanceOf((await alice.getAddress()))).to.be.bignumber.eq(ethers.utils.parseEther('40000'));
      expect(await huskyToken.balanceOf((await dev.getAddress()))).to.be.bignumber.eq(ethers.utils.parseEther('5000'));
    })
  })
});
