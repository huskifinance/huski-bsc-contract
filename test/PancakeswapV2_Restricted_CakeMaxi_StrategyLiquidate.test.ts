...
describe('PancakeswapV2CakeMaxiLiquidate', () => {
  ...
  /// Strategy instance(s)
  let strat: PancakeswapV2CakeMaxiLiquidate;
  ...
  let stratAsAlice: PancakeswapV2CakeMaxiLiquidate;
  let stratAsBob: PancakeswapV2CakeMaxiLiquidate;
  ...
  beforeEach(async () => {
    ...
    strat = await upgrades.deployProxy(PancakeswapV2CakeMaxiLiquidate, [routerV2.address], { unsafeAllow: ['delegatecall'] }) as PancakeswapV2CakeMaxiLiquidate;
    ...
    stratAsAlice = PancakeswapV2CakeMaxiLiquidate__factory.connect(strat.address, alice);
    stratAsBob = PancakeswapV2CakeMaxiLiquidate__factory.connect(strat.address, bob);
  ...
  context('When the base token is a wrap native', async () => {
          ...
          await expect(mockPancakeswapV2WorkerBNBFtokenPairAsAlice.work(
            ...
            ethers.utils.defaultAbiCoder.encode(
              ...
              [strat.address, ethers.utils.defaultAbiCoder.encode(
                ['uint256'],
                [ ethers.utils.parseEther('0.4993742177722153').add(1)]
              )],
            )
          )).to.be.revertedWith('PancakeswapV2CakeMaxiLiquidate::execute: insufficient baseToken amount received');
        });
      })
    ...
    context("When caller worker hasn't been whitelisted", async () => {
      it('should revert as bad worker', async () => {
        ...
        )).to.be.revertedWith('PancakeswapV2CakeMaxiLiquidate::onlyWhitelistedWorkers: bad worker');
      });
    })
        ...
        it('should revert', async () => {
          ...
          await expect(mockPancakeswapV2WorkerBaseFTokenPairAsAlice.work(
            ...
            ethers.utils.defaultAbiCoder.encode(
              ...
              [strat.address, ethers.utils.defaultAbiCoder.encode(
                ['uint256'],
                [ ethers.utils.parseEther('0.332499305557005937').add(1)]
              )],
            )
          )).to.be.revertedWith('PancakeswapV2CakeMaxiLiquidate::execute: insufficient baseToken amount received');
        });
