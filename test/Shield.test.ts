...
describe("Shield", () => {
  ...
  context("when migrate FairLaunchV1's owner from Timelock to Shield + Timelock", async() => {
    ...
    it('should revert when non owner to interact with Shield', async() => {
      await expect(
        shield.setHuskyPerBlock(ethers.utils.parseEther('1'))
  ...
  context("when adjust FairLaunchV1 params via Shield + Timelock", async() => {
    ...
    it('should set husky per block on FLV1 when Timelock is passed ETA', async() => {
      let eta = (await TimeHelpers.latest()).add(TimeHelpers.duration.days(ethers.BigNumber.from('4')));
      await timelockAsDev.queueTransaction(
        shield.address, '0', 'setHuskyPerBlock(uint256)',
        ethers.utils.defaultAbiCoder.encode(
          ['uint256'],
          [88]), eta
      );
      ...
      expect(await fairLaunch.huskyPerBlock()).to.be.bignumber.eq(88);
    });

    it('should allow to mint Husky if mintCount <= 8m', async() => {
      let eta = (await TimeHelpers.latest()).add(TimeHelpers.duration.days(ethers.BigNumber.from('4')));
      await timelockAsDev.queueTransaction(
      ...
      expect(await huskyToken.balanceOf(await alice.getAddress())).to.be.bignumber.eq(ethers.utils.parseEther('2750000'));
      ...
      expect(await huskyToken.balanceOf(await alice.getAddress())).to.be.bignumber.eq(ethers.utils.parseEther('7750000'));
    });
