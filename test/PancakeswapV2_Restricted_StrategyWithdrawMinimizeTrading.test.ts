...
describe('PancakeswapV2WithdrawMinimizeTrading', () => {
  ...
  /// Strategy instance(s)
  let strat: PancakeswapV2WithdrawMinimizeTrading;
  ...
  let stratAsAlice: PancakeswapV2WithdrawMinimizeTrading;
  let stratAsBob: PancakeswapV2WithdrawMinimizeTrading;
  ...
  beforeEach(async () => {
    ...
    farmingToken = await upgrades.deployProxy(MockERC20, ['FTOKEN', 'FTOKEN'], { unsafeAllow: ['delegatecall'] }) as MockERC20;
    ...
    mockPancakeswapBaseTokenWbnbV2Worker = await MockPancakeswapV2Worker.deploy(baseTokenWbnbLpV2.address, baseToken.address, wbnb.address) as MockPancakeswapV2Worker
    ...
    strat = await upgrades.deployProxy(PancakeswapV2WithdrawMinimizeTrading, 
    ...
    stratAsAlice = PancakeswapV2WithdrawMinimizeTrading__factory.connect(strat.address, alice);
    stratAsBob = PancakeswapV2WithdrawMinimizeTrading__factory.connect(strat.address, bob);
    ...
    mockPancakeswapBaseTokenWbnbV2WorkerAsBob = MockPancakeswapV2Worker__factory.connect(mockPancakeswapBaseTokenWbnbV2Worker.address, bob);
  ...
  context("When caller worker hasn't been whitelisted", async () => {
    it('should revert as bad worker', async () => {
      ...
      )).to.be.revertedWith('PancakeswapV2WithdrawMinimizeTrading::onlyWhitelistedWorkers: bad worker');
    });
  })
  ...
  context('It should convert LP tokens and farming token', () => {
    ...
    it('should revert, Bob uses withdraw minimize trading strategy to turn LPs back to farming with an unreasonable expectation', async () => {
      ...
      await expect(mockPancakeswapV2WorkerAsBob.work(
        0, await bob.getAddress(), ethers.utils.parseEther('1'),
        ethers.utils.defaultAbiCoder.encode(
          ['address', 'bytes'],
          [strat.address, ethers.utils.defaultAbiCoder.encode(
            ['uint256'],
      ...
      )).to.be.revertedWith('PancakeswapV2WithdrawMinimizeTrading::execute: insufficient farming tokens received')
    });
    ...
    it('should convert all LP tokens back to BaseToken and FTOKEN when debt < received BaseToken', async () => {
      ...
      await mockPancakeswapV2WorkerAsBob.work(
        0, await bob.getAddress(), ethers.utils.parseEther('0.5'),
        ethers.utils.defaultAbiCoder.encode(
          ['address', 'bytes'],
          [strat.address, ethers.utils.defaultAbiCoder.encode(
            ['uint256'],
    ...
    it('should convert all LP tokens back to BaseToken and farming token (debt > received BaseToken, farming token is enough to cover debt)', async () => {
      ...
      await mockPancakeswapV2WorkerAsBob.work(
        0, await bob.getAddress(), ethers.utils.parseEther('1.2'),
        ethers.utils.defaultAbiCoder.encode(
          ['address', 'bytes'],
          [strat.address, ethers.utils.defaultAbiCoder.encode(
            ['uint256'],
      ...
      expect(bobFtokenAfter.sub(bobFtokenBefore)).to.be.bignumber.eq(ethers.utils.parseEther('0.074937343358395989')); // 0.1 - 0.025 = 0.075 farming token
    });
    ...
    it('should revert when debt > received BaseToken, farming token is not enough to cover the debt', async () => {
      await expect(
        mockPancakeswapV2WorkerAsBob.work(
            0, await bob.getAddress(), ethers.utils.parseEther('3'),
            ethers.utils.defaultAbiCoder.encode(
              ['address', 'bytes'],
              [strat.address, ethers.utils.defaultAbiCoder.encode(
                ['uint256'],
  ...
  context('It should handle properly when the farming token is WBNB', () => {
    beforeEach(async () => {
      ...
      expect(await baseTokenWbnbLpV2.balanceOf(await bob.getAddress())).to.be.bignumber.eq(ethers.utils.parseEther('0.316227766016837933'));
      await baseTokenWbnbLpV2AsBob.transfer(mockPancakeswapBaseTokenWbnbV2Worker.address, ethers.utils.parseEther('0.316227766016837933'));
    });
    ...
    it('should convert all LP tokens back to BaseToken and BNB, while debt == received BaseToken', async () => {
      ...
      expect(await baseTokenWbnbLpV2.balanceOf(strat.address)).to.be.bignumber.eq(ethers.utils.parseEther('0'));
      expect(await baseTokenWbnbLpV2.balanceOf(await bob.getAddress())).to.be.bignumber.eq(ethers.utils.parseEther('0'));
      expect(bobBaseTokenAfter.sub(bobBaseTokenBefore)).to.be.bignumber.eq(ethers.utils.parseEther('1'));
    ...
    it('should convert all LP tokens back to BaseToken and BNB (debt > received BaseToken, BNB is enough to cover debt)', async () => {
      ...
      expect(await lpV2.balanceOf(await bob.getAddress())).to.be.bignumber.eq(ethers.utils.parseEther('0'));
      expect(bobBtokenAfter.sub(bobBtokenBefore)).to.be.bignumber.eq(ethers.utils.parseEther('1.2'));
      expect(bobBnbAfter.sub(bobBnbBefore)).to.be.bignumber.eq(ethers.utils.parseEther('0.074937343358395989'));
    });

    it('should revert when debt > received BaseToken, BNB is not enough to cover the debt', async () => {
        await expect(mockPancakeswapBaseTokenWbnbV2WorkerAsBob.work(
            0, await bob.getAddress(), ethers.utils.parseEther('3'),
            ethers.utils.defaultAbiCoder.encode(
                ['address', 'bytes'],
                [strat.address, ethers.utils.defaultAbiCoder.encode(
