...
contract HuskyToken is ERC20("HuskiToken", "HUSKI"), Ownable {
  ...
  function canUnlockAmount(address _account) public view returns (uint256) {
    // When block number less than startReleaseBlock, no HUSKYs can be unlocked
    if (block.number < startReleaseBlock) {
      return 0;
    }
    // When block number more than endReleaseBlock, all locked HUSKYs can be unlocked
    else if (block.number >= endReleaseBlock) {
      return _locks[_account];
    }
    // When block number is more than startReleaseBlock but less than endReleaseBlock,
    // some HUSKYs can be released
    else
    {
  ...
  function unlock() external {
    require(_locks[msg.sender] > 0, "no locked HUSKYs");
  ...
  // @dev move HUSKYs with its locked funds to another account
  ...
  function lockToOwner(address _account, uint256 _amount) external onlyOwner {
    require(_account != address(0), "no lock to address(0)");
    require(_amount <= balanceOf(_account), "no lock over balance");
  ...
  function delegateBySig(
    ...
    require(signatory != address(0), "HUSKY::delegateBySig: invalid signature");
    require(nonce == nonces[signatory]++, "HUSKY::delegateBySig: invalid nonce");
    require(now <= expiry, "HUSKY::delegateBySig: signature expired");
  ...
  function getPriorVotes(address account, uint256 blockNumber) external view returns (uint256) {
    require(blockNumber < block.number, "HUSKY::getPriorVotes: not yet determined");
  ...
  function _delegate(address delegator, address delegatee) internal {
    ...
    uint256 delegatorBalance = balanceOf(delegator); // balance of underlying HUSKYs (not scaled);
  ...
  function _writeCheckpoint(
    ...
    uint32 blockNumber = safe32(block.number, "HUSKY::_writeCheckpoint: block number exceeds 32 bits");
