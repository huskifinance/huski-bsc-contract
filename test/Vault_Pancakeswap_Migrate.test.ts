...
//   PancakeswapV2AddBaseTokenOnly,
//   PancakeswapV2AddBaseTokenOnly__factory,
//   PancakeswapV2AddTwoSidesOptimalMigrate,
...
//   let liqStratV2: PancakeswapV2Liquidate;
...
//     complexPriceOracle = await upgrades.deployProxy(ComplexPriceOracle, [await deployer.getAddress()]) as ComplexPriceOracle;
...
//       liqStratV2 = await upgrades.deployProxy(PancakeswapV2Liquidate, [routerV2.address]) as PancakeswapV2Liquidate;
...
//         // PancakeswapV2Worker needs to be updated to PancakeswapV2WorkerMigrate
//         const PancakeswapV2WorkerMigrate = (await ethers.getContractFactory(
//           'PancakeswapV2WorkerMigrate',
//           deployer
//         )) as PancakeswapV2WorkerMigrate__factory
//         const pancakeswapV2workerMigrate = await upgrades.upgradeProxy(PancakeswapV2Worker.address, PancakeswapV2WorkerMigrate) as PancakeswapV2WorkerMigrate
...
//         expect(await PancakeswapV2Worker.okStrats(addStrat.address)).to.be.eq(false)
//         expect(await PancakeswapV2Worker.okStrats(liqStrat.address)).to.be.eq(false)
//         expect(await PancakeswapV2Worker.okStrats(partialCloseStrat.address)).to.be.eq(false)

//         // expect that all new strats must be enabled
//         expect(await PancakeswapV2Worker.okStrats(addStratV2.address)).to.be.eq(true)
//         expect(await PancakeswapV2Worker.okStrats(liqStratV2.address)).to.be.eq(true)
...
//         [workerLPBefore, workerDebtBefore] = await masterChef.userInfo(lpV2poolId, PancakeswapV2Worker.address);
//         await PancakeswapV2WorkerAsEve.reinvest();
...
//         [workerLPAfter, workerDebtAfter] = await masterChef.userInfo(lpV2poolId, PancakeswapV2Worker.address);
//         // LP tokens of worker should be inceased from reinvestment
//         expect(workerLPAfter).to.be.bignumber.gt(workerLPBefore);
...
//             ]
//           )
//         )).to.be.revertedWith('PancakeswapV2Worker::work: unapproved work strategy');
...
//         const [stakedLPv2, ] = await masterChef.userInfo(lpV2poolId, PancakeswapV2Worker.address)
//         const [stakedLPv1, ] = await masterChef.userInfo(poolId, PancakeswapV2Worker.address)
...
//           deployer
//         )) as PancakeswapV2Worker__factory
//         const pancakeswapV2worker = await upgrades.upgradeProxy(PancakeswapV2Worker.address, PancakeswapV2Worker) as PancakeswapV2Worker
//         await pancakeswapV2worker.deployed()
...
//         const restrictedAddStrat = await upgrades.deployProxy(PancakeswapV2AddBaseTokenOnly, [routerV2.address]) as PancakeswapV2AddBaseTokenOnly
...
//           deployer
//         )) as PancakeswapV2Liquidate__factory;
//         const restrictedLiqStrat = await upgrades.deployProxy(PancakeswapV2Liquidate, [routerV2.address]) as PancakeswapV2Liquidate;
...
//           addStratV2.address,
//           liqStratV2.address,
//           [], [addStrat.address, liqStrat.address, partialCloseStrat.address])).to.be.reverted;

//         // Check Bob position info
//         // Bob health after clean should eqaul to after migrate
//         // Bob's debt must still the same
//         let [bobHealthAfterClean, bobDebtClean] = await vault.positionInfo('1');
//         expect(bobHealthAfterClean).to.be.bignumber.eq(bobHealthAfterMigrate);
//         expect(bobDebtClean).to.be.bignumber.eq(bobDebt);
