...
contract BasicVaultConfig is IVaultConfig, OwnableUpgradeSafe {
  ...
  /// The prize for successfully closing a position to the reserve pool.
  uint256 public override getCloseBps;
  ...
  function initialize(
    ...
    maxKillBps = 500;
    setParams(
      _minDebtSize, _reservePoolBps, _closeBps, _killBps, _interestModel, _wrappedNative, _wNativeRelayer, _fairLaunch);
  }
  ...
  function setParams(
    ...
    require(_killBps <= maxKillBps, "BasicVaultConfig::setParams: kill bps exceeded max kill bps");

    minDebtSize = _minDebtSize;
    getReservePoolBps = _reservePoolBps;
    getCloseBps = _closeBps;
    getKillBps = _killBps;
    interestModel = _interestModel;
  ...
  function setWorkers(address[] calldata addrs, IWorkerConfig[] calldata configs) external onlyOwner {
    require(addrs.length == configs.length, "BasicVaultConfig::setWorkers: bad length");
