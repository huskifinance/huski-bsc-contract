...
describe('PancakeswapV2AddBaseTokenOnly', () => {
  ...
  /// Strategy instance(s)
  let strat: PancakeswapV2AddBaseTokenOnly;
  ...
  let stratAsAlice: PancakeswapV2AddBaseTokenOnly;
  let stratAsBob: PancakeswapV2AddBaseTokenOnly;
  ...
  beforeEach(async () => {
    ...
    strat = await upgrades.deployProxy(PancakeswapV2AddBaseTokenOnly, [routerV2.address], { unsafeAllow: ['delegatecall'] }) as PancakeswapV2AddBaseTokenOnly;
    ...
    stratAsAlice = PancakeswapV2AddBaseTokenOnly__factory.connect(strat.address, alice);
    stratAsBob = PancakeswapV2AddBaseTokenOnly__factory.connect(strat.address, bob);
  ...
  context('When contract get LP < minLP', async () => {
    it('should revert', async () => {
      ...
      )).to.be.revertedWith('PancakeswapV2AddBaseTokenOnly::execute: insufficient LP tokens received');
    });
  })
  ...
  context("When caller worker hasn't been whitelisted", async () => {
    it('should revert as bad worker', async () => {
      ...
      )).to.be.revertedWith('PancakeswapV2AddBaseTokenOnly::onlyWhitelistedWorkers: bad worker');
    });
  })
