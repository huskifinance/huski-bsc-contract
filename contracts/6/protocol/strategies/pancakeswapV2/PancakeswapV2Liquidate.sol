...
contract PancakeswapV2Liquidate is OwnableUpgradeSafe, ReentrancyGuardUpgradeSafe, IStrategy {
  ...
  modifier onlyWhitelistedWorkers() {
    require(okWorkers[msg.sender], "PancakeswapV2Liquidate::onlyWhitelistedWorkers: bad worker");
    _;
  } 
    ...
    nonReentrant
  {
    ...
    require(lpToken.approve(address(router), uint256(-1)), "PancakeswapV2Liquidate::execute: unable to approve LP token");
    ...
    require(balance >= minBaseToken, "PancakeswapV2Liquidate::execute: insufficient baseToken received");
    SafeToken.safeTransfer(baseToken, msg.sender, balance);
    // 6. Reset approve for safety reason
    require(lpToken.approve(address(router), 0), "PancakeswapV2Liquidate::execute: unable to reset LP token approval");
