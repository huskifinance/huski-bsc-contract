...
describe('PancakeswapV2CakeMaxiAddBaseTokenOnly', () => {
  ...
  /// Strategy instance(s)
  let strat: PancakeswapV2CakeMaxiAddBaseTokenOnly;
  ...
  let stratAsAlice: PancakeswapV2CakeMaxiAddBaseTokenOnly;
  let stratAsBob: PancakeswapV2CakeMaxiAddBaseTokenOnly;
  ...
  beforeEach(async () => {
    ...
    farmingToken = await upgrades.deployProxy(MockERC20, 
      ['FTOKEN', 'FTOKEN'], { unsafeAllow: ['delegatecall'] }) as MockERC20;
    ...
    strat = await upgrades.deployProxy(PancakeswapV2CakeMaxiAddBaseTokenOnly, 
      [routerV2.address], { unsafeAllow: ['delegatecall'] }) as PancakeswapV2CakeMaxiAddBaseTokenOnly;
    ...
    stratAsAlice = PancakeswapV2CakeMaxiAddBaseTokenOnly__factory.connect(strat.address, alice);
    stratAsBob = PancakeswapV2CakeMaxiAddBaseTokenOnly__factory.connect(strat.address, bob);
  ...
  context('when the base token is a wrap native', async () => {
    context('When contract get farmingToken amount < minFarmingTokenAmount', async () => {
        it('should revert', async () => {
          ...
          )).to.be.revertedWith('PancakeswapV2CakeMaxiAddBaseTokenOnly::execute: insufficient farmingToken amount received');
        });
      })
    ...
    context("When caller worker hasn't been whitelisted", async () => {
      it('should revert as bad worker', async () => {
        ...
        )).to.be.revertedWith('PancakeswapV2CakeMaxiAddBaseTokenOnly::onlyWhitelistedWorkers: bad worker');
      });
    })
  ...
  context('when the base token is not a wrap native', async () => {
    ...
    it('should convert ALL baseToken (WBTC) to farmingToken', async () => {
      ...
      expect(await farmingToken.balanceOf(mockPancakeswapV2WorkerBaseFTokenPair.address)).to.be.bignumber.eq(ethers.utils.parseEther('0.014227298611701251'))
      expect(await farmingToken.balanceOf(strat.address)).to.be.bignumber.eq(ethers.utils.parseEther('0'))
      expect(await baseToken.balanceOf(mockPancakeswapV2WorkerBaseFTokenPair.address)).to.be.bignumber.eq(ethers.utils.parseEther('0'))
    });
  })
