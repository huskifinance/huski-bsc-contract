...
import {
  ...
  PancakeswapV2AddTwoSidesOptimal__factory,
...
describe('Pancakeswapv2RestrictedStrategyAddTwoSideOptimal', () => {
  ...
  let addRestrictedStrat: PancakeswapV2AddTwoSidesOptimal
  let addRestrictedStratAsDeployer: PancakeswapV2AddTwoSidesOptimal;

  // Contract Signer
  let addRestrictedStratAsBob: PancakeswapV2AddTwoSidesOptimal;
  ...
  const setupRestrictedTest = async () => {
    ...
    mockedVault = await upgrades.deployProxy(MockVaultForRestrictedAddTwosideOptimalStrat, { unsafeAllow: ['delegatecall'] }) as MockVaultForRestrictedAddTwosideOptimalStrat;
    await mockedVault.deployed();

    const PancakeswapV2AddTwoSidesOptimal = (await ethers.getContractFactory(
      "PancakeswapV2AddTwoSidesOptimal",
      deployer
    )) as PancakeswapV2AddTwoSidesOptimal__factory;
    addRestrictedStrat = await upgrades.deployProxy(PancakeswapV2AddTwoSidesOptimal, [routerV2.address, mockedVault.address], { unsafeAllow: ['delegatecall'] }) as PancakeswapV2AddTwoSidesOptimal
  ...
  const setupContractSigner = async () => {
     // Contract signer
     addRestrictedStratAsBob = PancakeswapV2AddTwoSidesOptimal__factory.connect(addRestrictedStrat.address, bob);
     addRestrictedStratAsDeployer = PancakeswapV2AddTwoSidesOptimal__factory.connect(addRestrictedStrat.address, deployer)
  ...
  describe('full flow test', async () => {
    ...
    it('should convert all BTOKEN to LP tokens at best rate', async () => {
      ...
      Assert.assertAlmostEqual((await lpV2.balanceOf(mockPancakeswapV2Worker.address)).toString(), ethers.utils.parseEther('0.240242902746503337').toString())
    ...
    it('should convert some BTOKEN and some FTOKEN to LP tokens at best rate', async () => {
      ...
      Assert.assertAlmostEqual((await lpV2.balanceOf(mockPancakeswapV2Worker.address)).toString(), ethers.utils.parseEther('1.8261834917016726').toString())
      expect(await lpV2.balanceOf(mockPancakeswapV2Worker.address)).to.above(stratLPBalance);
      expect(await farmingToken.balanceOf(addRestrictedStrat.address)).to.be.bignumber.below(MAX_ROUNDING_ERROR);
    })
  })
  ...
  describe('restricted test', async() => {
    ...
    context('When the caller worker is not whitelisted', async () => {
      it ('should revert', async() => {
        ...
        ))).to.revertedWith('PancakeswapV2AddTwoSidesOptimal::onlyWhitelistedWorkers: bad worker')
      })
    })
