...
import {
  ...
  PancakeRouterV2,
  PancakeswapV2CakeMaxiWithdrawMinimizeTrading,
  PancakeswapV2CakeMaxiWithdrawMinimizeTrading__factory,
  PancakeswapV2CakeMaxiAddBaseTokenOnly,
  PancakeswapV2CakeMaxiAddBaseTokenOnly__factory,
  PancakeswapV2CakeMaxiAddBaseWithFarm,
  PancakeswapV2CakeMaxiAddBaseWithFarm__factory,
  PancakeswapV2CakeMaxiLiquidate,
  PancakeswapV2CakeMaxiLiquidate__factory,
...
describe('CakeMaxiWorker', () => {
  ...
  let husky: HuskyToken;
  let cake: CakeToken;
  let syrup: SyrupBar;

  /// Strategy instance(s)
  let stratAdd: PancakeswapV2CakeMaxiAddBaseTokenOnly
  let stratLiq: PancakeswapV2CakeMaxiLiquidate
  let stratAddWithFarm: PancakeswapV2CakeMaxiAddBaseWithFarm
  let stratMinimize: PancakeswapV2CakeMaxiWithdrawMinimizeTrading;
  let stratEvil: PancakeswapV2CakeMaxiWithdrawMinimizeTrading
  ...
  beforeEach(async () => {
    ...
    husky = await HuskyToken.deploy(132, 137);
    await husky.deployed();
    await husky.mint(await deployer.getAddress(), ethers.utils.parseEther('1000'));
    ...
    await factoryV2.createPair(husky.address, wbnb.address);
    ...
    // add beneficial vault with husky as an underlying token, thus beneficialVault reward is HUSKI
    const MockBeneficialVault =  (await ethers.getContractFactory(
      "MockBeneficialVault",
      deployer
      )) as MockBeneficialVault__factory;
    mockedBeneficialVault = await upgrades.deployProxy(MockBeneficialVault, [husky.address], { unsafeAllow: ['delegatecall'] }) as MockBeneficialVault;
    await mockedBeneficialVault.deployed();
    await mockedBeneficialVault.setMockOwner(await alice.getAddress())
    // Setup Strategies
    const PancakeswapV2CakeMaxiAddBaseTokenOnly = (await ethers.getContractFactory(
      "PancakeswapV2CakeMaxiAddBaseTokenOnly",
      deployer
    )) as PancakeswapV2CakeMaxiAddBaseTokenOnly__factory;
    stratAdd = await upgrades.deployProxy(PancakeswapV2CakeMaxiAddBaseTokenOnly, [routerV2.address], { unsafeAllow: ['delegatecall'] }) as PancakeswapV2CakeMaxiAddBaseTokenOnly;
    await stratAdd.deployed();
    const PancakeswapV2CakeMaxiAddBaseWithFarm = (await ethers.getContractFactory(
      "PancakeswapV2CakeMaxiAddBaseWithFarm",
      deployer
    )) as PancakeswapV2CakeMaxiAddBaseWithFarm__factory;
    stratAddWithFarm = await upgrades.deployProxy(PancakeswapV2CakeMaxiAddBaseWithFarm, [routerV2.address, mockedVault.address], { unsafeAllow: ['delegatecall'] }) as PancakeswapV2CakeMaxiAddBaseWithFarm;
    await stratAddWithFarm.deployed();
    const PancakeswapV2CakeMaxiLiquidate = (await ethers.getContractFactory(
      "PancakeswapV2CakeMaxiLiquidate",
      deployer
    )) as PancakeswapV2CakeMaxiLiquidate__factory;
    stratLiq = await upgrades.deployProxy(PancakeswapV2CakeMaxiLiquidate, [routerV2.address], { unsafeAllow: ['delegatecall'] }) as PancakeswapV2CakeMaxiLiquidate;
    await stratLiq.deployed();
    const PancakeswapV2CakeMaxiWithdrawMinimizeTrading = (await ethers.getContractFactory(
      "PancakeswapV2CakeMaxiWithdrawMinimizeTrading",
      deployer
    )) as PancakeswapV2CakeMaxiWithdrawMinimizeTrading__factory;
    stratMinimize = await upgrades.deployProxy(PancakeswapV2CakeMaxiWithdrawMinimizeTrading, 
      [routerV2.address, wNativeRelayer.address], 
      { unsafeAllow: ['delegatecall'] }) as PancakeswapV2CakeMaxiWithdrawMinimizeTrading;
    await stratMinimize.deployed();
    const EvilStrat = (await ethers.getContractFactory(
      "PancakeswapV2CakeMaxiWithdrawMinimizeTrading",
      deployer
    )) as PancakeswapV2CakeMaxiWithdrawMinimizeTrading__factory;
    stratEvil = await upgrades.deployProxy(EvilStrat, [routerV2.address, wNativeRelayer.address], { unsafeAllow: ['delegatecall'] }) as PancakeswapV2CakeMaxiWithdrawMinimizeTrading;
    ...
    cakeMaxiWorkerNative = await upgrades.deployProxy(CakeMaxiWorker, 
      [await alice.getAddress(), wbnb.address, masterChef.address, 
        routerV2.address, mockedBeneficialVault.address, poolId, 
        stratAdd.address, stratLiq.address, REINVEST_BOUNTY_BPS, 
        ZERO_BENEFICIALVAULT_BOUNTY_BPS], 
        { unsafeAllow: ['delegatecall'] }) as CakeMaxiWorker
    await cakeMaxiWorkerNative.deployed();
    cakeMaxiWorkerNonNative = await upgrades.deployProxy(CakeMaxiWorker, 
      [await alice.getAddress(), baseToken.address, masterChef.address, 
        routerV2.address,  mockedBeneficialVault.address, poolId, 
        stratAdd.address, stratLiq.address, REINVEST_BOUNTY_BPS, 
        ZERO_BENEFICIALVAULT_BOUNTY_BPS], 
        { unsafeAllow: ['delegatecall'] }) as CakeMaxiWorker
    ...
    fairLaunch = await FairLaunch.deploy(
      husky.address, (await deployer.getAddress()), HUSKI_REWARD_PER_BLOCK, 0, HUSKI_BONUS_LOCK_UP_BPS, 0
    );
    await fairLaunch.deployed();

    await husky.transferOwnership(fairLaunch.address);
    ...
    integratedVault = await upgrades.deployProxy(Vault, 
      [simpleVaultConfig.address, wbnb.address, 'Interest Bearing BTOKEN', 
      'ibBTOKEN', 18, debtToken.address], { unsafeAllow: ['delegatecall'] }) as Vault;
    ...
    // badgeRelayer = await upgrades.deployProxy(BadgeRelayer,[huskyToken.address,fairLaunch.address,badgeNFT.address,badgePoints.address,complexPriceOracle.address,simpleVaultConfig.address]) as BadgeRelayer;
    ...
    await complexPriceOracle.setPrices([await integratedVault.debtToken()],[1])
    ...
    integratedCakeMaxiWorker = await upgrades.deployProxy(CakeMaxiWorker, 
      [integratedVault.address, wbnb.address, masterChef.address, 
        routerV2.address,  integratedVault.address, poolId, stratAdd.address, 
        stratLiq.address, REINVEST_BOUNTY_BPS, ZERO_BENEFICIALVAULT_BOUNTY_BPS], 
        { unsafeAllow: ['delegatecall'] }) as CakeMaxiWorker
    await cakeMaxiWorkerNonNative.deployed();
    ...
    await husky.approve(routerV2.address, ethers.utils.parseEther('10'));
    ...
    // Add liquidity to the FTOKEN-HUSKI pool on Pancakeswap
    await routerV2.addLiquidity(
      wbnb.address, husky.address,
  ...
  describe("#reinvest()", async() => {
    ...
    context("When the reinvestor reinvest in the middle of a transaction set", async () => {
      context('When beneficialVaultBounty takes 0% of reinvest bounty', async () => {
        it('should increase the size of total balance, bounty is sent to the reinvestor', async () => {
          ...
          Assert.assertAlmostEqual((await husky.balanceOf(mockedBeneficialVault.address)).toString(), ethers.utils.parseEther('0').toString())
          userInfo = await masterChef.userInfo(0, cakeMaxiWorkerNative.address)
          // Bob start opening his position using 0.1 wbnb
          // amountOut of 0.1 will be
          // if 1.2 WBNB = (0.1 - (0.00907024323709934 + 0.0075601110540523785)) FToken
      ...
      context('When beneficialVaultBounty takes 10% of reinvest bounty', async () => {
        it('should increase the size of total balance, bounty is sent to the reinvestor and beneficial vault based on a correct bps', async () => {
          ...
          // 10% of 0.003 (0.0003) will be distributed to the vault by swapping 0.003 of reward token into a beneficial vault token (this is scenario, it will be HUSKI)
          ...
          // if 10WBNB = 10HUSKI
          // 0.004291917527507464 WBNB =  (0.004291917527507464 * 0.9975 * 10) / (10  + 0.004291917527507464 * 0.9975) = 0.004279355661192217 HUSKI
          userInfo = await masterChef.userInfo(0, cakeMaxiWorkerNative.address)
          await cakeMaxiWorkerNativeAsEve.reinvest()
          Assert.assertAlmostEqual((await cake.balanceOf(await eve.getAddress())).toString(), ethers.utils.parseEther('0.0027').toString())
          Assert.assertAlmostEqual((await husky.balanceOf(mockedBeneficialVault.address)).toString(), ethers.utils.parseEther('0.004279355661192217').toString())
          userInfo = await masterChef.userInfo(0, cakeMaxiWorkerNative.address)
          // Bob start opening his position using 0.1 wbnb
          // amountOut of 0.1 will be
    ...
    context("When integrated with an actual vault", async () => {
      it('should reinvest with updated beneficial vault reward to the beneficial vault', async () => {
        ...
        // 10% of 0.002 (0.0002) will be distributed to the vault by swapping 0.002 of reward token into a beneficial vault token (this is scenario, it will be HUSKI)
        // if (0.1 - (0.00907024323709934 + 0.0075601110540523785)) FToken = 1.2 WBNB
        // 0.08336964570884828 FToken = 1.2 WBNB
