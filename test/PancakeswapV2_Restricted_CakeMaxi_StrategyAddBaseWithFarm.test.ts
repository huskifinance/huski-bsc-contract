...
describe('PancakeswapV2CakeMaxiAddBaseWithFarm', () => {
  ...
  /// Strategy instance(s)
  let strat: PancakeswapV2CakeMaxiAddBaseWithFarm;
  ...
  let stratAsAlice: PancakeswapV2CakeMaxiAddBaseWithFarm;
  let stratAsBob: PancakeswapV2CakeMaxiAddBaseWithFarm;
  ...
  beforeEach(async () => {
    ...
    strat = await upgrades.deployProxy(PancakeswapV2CakeMaxiAddBaseWithFarm, 
      [routerV2.address, mockedVault.address], { unsafeAllow: ['delegatecall'] }) as PancakeswapV2CakeMaxiAddBaseWithFarm;
    ...
    stratAsAlice = PancakeswapV2CakeMaxiAddBaseWithFarm__factory.connect(strat.address, alice);
    stratAsBob = PancakeswapV2CakeMaxiAddBaseWithFarm__factory.connect(strat.address, bob);
  ...
  context('when the base token is a wrap native', async () => {
    context('when contract get farmingToken amount (from input base token) < minFarmingTokenAmount', async () => {
        it('should revert', async () => {
          ...
          )).to.be.revertedWith('PancakeswapV2CakeMaxiAddBaseWithFarm::execute: insufficient farmingToken amount received');
        });
    })
    ...
    context('When contract get farmingToken amount (from input base token) >= minFarmingTokenAmount', async () => {
        it('should not revert', async () => {
            ...
            )).not.to.be.revertedWith('PancakeswapV2CakeMaxiAddBaseWithFarm::execute: insufficient farmingToken amount received');
        });
    })
    ...
    context("When caller worker hasn't been whitelisted", async () => {
      it('should revert as bad worker', async () => {
        ...
        )).to.be.revertedWith('PancakeswapV2CakeMaxiAddBaseWithFarm::onlyWhitelistedWorkers: bad worker');
      });
    })
