...
describe('PancakeswapV2Liquidate', () => {
  ...
  /// Strategy instance(s)
  let strat: PancakeswapV2Liquidate;
  ...
  let stratAsAlice: PancakeswapV2Liquidate;
  let stratAsBob: PancakeswapV2Liquidate;
  ...
  beforeEach(async () => {
    ...
    strat = await upgrades.deployProxy(PancakeswapV2Liquidate, [routerV2.address], { unsafeAllow: ['delegatecall'] }) as PancakeswapV2Liquidate;
    ...
    stratAsAlice = PancakeswapV2Liquidate__factory.connect(strat.address, alice);
    stratAsBob = PancakeswapV2Liquidate__factory.connect(strat.address, bob);
  ...
  context("When caller worker hasn't been whitelisted", async () => {
    it('should revert as bad worker', async () => {
      ...
      )).to.be.revertedWith('PancakeswapV2Liquidate::onlyWhitelistedWorkers: bad worker');
    });
  })
